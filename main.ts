...
const hb_utils = require('handlebars-utils');
...
function convertCsv(source: string) {
	...
	const csv = Papa.parse(source, { header: true, skipEmptyLines: true });
...
export default class JsonImport extends Plugin {
	...
	hb_table(...args: any) {
		...
		if (args.length < 4) return ''; // string val1 result1 options
		if (args[0] == undefined || args[0] == null) return args[0];
		const len = args.length - 1;
		const options = args[len];
		let value = args[0].toString();
		for (let i = 1; i < len; i += 2) {
			const result = value.match(RegExp(`^${args[i]}$`, 'u'));
			if (result) {
				value = args[i + 1];
				...
				if (result.length > 1) {
					...
					value = value.replaceAll(/\$(\d+)/g, (match: string, p1: string) => {
						const param = +p1; // first parameter = 1
	...
	hb_substring(...args: any) {
		const len = args.length - 1;
		const options = args[len];
		let value = args[0];
		if (len === 3) {
			// fourth parameter = options
			const beginPos = args[1];
			const length = args[2];
			if (
				typeof value === 'string' &&
				typeof beginPos === 'number' &&
	...
	hb_strarray(...args: any) {
		const len = args.length - 1;
		const options = args[len];
		const orig = args[0];
		if (args.length != 2 || typeof orig !== 'string')
			return hb_utils.value(orig, this, options);
		return hb_utils.value([...orig], this, options);
	}

	// {{replacereg orig "regular expression" "replacement"}}
	hb_replacereg(...args: any) {
		if (args.length != 4) return args[0];
		const orig: string = args[0];
		const pattern = RegExp(args[1], 'g');
	...
	hb_strsplit(...args: any) {
		if (args.length != 3) return args[0];
		const orig: string = args[0];
		const pattern: RegExp = args[1]; // string or RegExp
	...
	async checkPath(filename: string) {
		...
		const path = filename.slice(0, pos);
		...
		if (!exists) {
			...
			await this.app.vault
				.createFolder(path)
				.catch((err) => console.log(`app.vault.checkPath: ${err}`));
		}
	...
	async generateNotes(
		...
		if (helperfile) {
			const initJsonHelpers = new Function('handlebars', await helperfile.text());
		...
		if (!Array.isArray(topobj) && forcearray) topobj = [topobj];
		...
		//}
		const entries: any = Array.isArray(topobj)
			? topobj.entries()
		...
		for (const [index, row] of entries) {
			...
			if (body.contains('[object Object]')) {
				...
				new Notice(
					`Incomplete conversion for '${notefile}'. Look for '[object Object]' (also reported in console)`
				);
			}

			const filename = topfolder + '/' + this.validFilename(notefile) + '.md';
			await this.checkPath(filename);
			// Delete the old version, if it exists
			const exist = this.app.vault.getAbstractFileByPath(filename);
			if (exist) {
				if (!overwrite && !append) {
					new Notice(
					...
					await this.app.vault
						...
						.catch((err) => console.log(`app.vault.append: ${err}`));
				} else {
					await this.app.vault
						.delete(exist)
						.catch((err) => console.log(`app.vault.delete: ${err}`));
					await this.app.vault
						.create(filename, body)
						.catch((err) => console.log(`app.vault.create: ${err}`));
				}
			} else {
				await this.app.vault
					.create(filename, body)
...
class FileSelectionModal extends Modal {
	...
	setHandler(caller: object, handler: (...args: any[]) => any): void {
		this.caller = caller;
	...
	onOpen() {
		const setting1 = new Setting(this.contentEl)
			.setName('Choose JSON/CSV File')
			.setDesc(
				'Choose JSON/CSV data file to import, or paste text into the text box'
			);
		...
		const setting2 = new Setting(this.contentEl)
			.setName('Choose TEMPLATE File')
			.setDesc('Choose the Template (Handlebars) file');
		...
		const setting2a = new Setting(this.contentEl)
			.setName('Choose HELPERS File')
			.setDesc(
				'Optionally select a file containing some Handlebars Helpers functions'
			);
		...
		const setting3b = new Setting(this.contentEl)
			.setName('Field containing the data')
			.setDesc(
				'The field containing the array of data (leave blank to use entire file)'
			);
		...
		const setting3c = new Setting(this.contentEl)
			.setName('Each subfield is a separate note')
			.setDesc(
				"Select this option if 'Field containing the data' is a single object and a separate note should be created for each field of that object."
			);
		...
		const setting3 = new Setting(this.contentEl)
			.setName('Field to use as Note name')
			.setDesc(
				'Field in each row of the JSON/CSV data to be used for the note name'
			);
		...
		const settingPrefix = new Setting(this.contentEl)
			.setName('Note name prefix/suffix')
			.setDesc(
				'Optional prefix/suffix to be added either side of the value from the above Note name field'
			);
		...
		const setting3a = new Setting(this.contentEl)
			.setName('Allow paths in Note name')
			.setDesc(
				'Allow / in the Note name field to be used to create folders (when not selected / will be replaced by _ as part of note name)'
			);
		...
		const settingOverwrite = new Setting(this.contentEl)
			.setName('Overwrite existing Notes')
			.setDesc(
				'When ticked, existing Notes with a matching name will be overwritten by entries in the supplied JSON/CSV file. (If not ticked, then existing Notes will remain unchanged and entries in the JSON/CSV file with a matching name will be ignored)'
			);
		...
		const setting4 = new Setting(this.contentEl)
			.setName('Name of Destination Folder in Vault')
			.setDesc(
				'The name of the folder in your Obsidian Vault, which will be created if required'
			);
		...
		const setting5 = new Setting(this.contentEl)
			.setName('Import')
		...
		input5.onclick = async () => {
			...
			if (srctext.length == 0) {
				...
				for (let i = 0; i < datafiles.length; i++) {
					...
					const is_json: boolean = datafiles[i].name.endsWith('.json');
